import { MOVE, COUNT, PLAY, RESET, SHUFFLE } from '../actions/actions';
...
function array_shuffle(arg) {
	const sizeLines = arg.length - 1;
	const sizeCols = arg[0].length - 1;

	let grid = arg.map((lines) => lines.map((x) => x));
		...
		line.forEach((column, colIndex) => {
			if (column === -1) {
				...
				if (
					...
					let grid = newObjectReturn.grid.map((x) =>
						x instanceof Array ? x.map((i) => i) : x
					);
